...
open_greenery::database::IrrigationConfigWriter::IrrigationConfigWriter(Table _table)
...
void open_greenery::database::IrrigationConfigWriter::write(IrrigationConfigRecord _record)
{
    ...
    const auto query = std::string("INSERT INTO ")
            ...
            + "\"" + _record.soil_moisture_reader.table().name + "\")";

    int rv = table().database->exec(query);
    if (rv != 1)
    {
        throw std::runtime_error("Database Error: Insert to the " + table().name + " returned " + std::to_string(rv));
    }
    transaction.commit();
}
